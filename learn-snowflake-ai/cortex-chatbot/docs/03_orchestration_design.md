...
| パターン | 開始ツール | 終了ツール | 典型的な質問の形 |
|---|---|---|---|
| A: Analyst のみ | Analyst | - | 「〇〇テーブルを使うジョブ一覧」「〇〇関数を使うファイルは何件？」 |
| B: Search のみ | Search | - | 「〇〇.sqlの処理内容」「このエラーの原因」 |
| C: Analyst → Search | Analyst | Search | 「〇〇テーブルを更新しているジョブの**コードを解説して**」 |
| D: Search → Analyst | Search | Analyst | 「〇〇ファイルのカラム変更が**影響する機能**は？」 |
...
1. impact_analysis_tool (Cortex Analyst):
   構造化メタデータへの検索・集計が必要な場合に使用してください。
...
2. source_code_search_tool (Cortex Search):
   ソースコードの内容理解・推論が必要な場合に使用してください。
   - 「〇〇.sqlはどんな処理をするプログラムですか？」
   - 「〇〇モジュールの処理内容を教えて」
   - 「このエラーが出た場合の原因と対処法は？」
   - 「〇〇機能の処理フローを解説して」
   - 「〇〇関数を使っているプログラムはどんな処理をしているか？」（件数でなく内容が目的）
   - 「〇〇ファイルの〇〇カラム変更で影響するプログラムを教えて」
     （ソースコードを読んでカラム処理を追う必要があるため Search が先）
   - ソースコードの内容・意図・処理ロジック・変数マッピングに関する質問

3. 両ツール使用（Analyst → Search）:
   構造検索でジョブを特定した後にコード内容を説明する場合に使用。
   - 「〇〇テーブルを更新しているジョブのソースコードを説明して」
     → まずimpact_analysis_toolで該当ジョブを特定し、
       次にsource_code_search_toolでそのジョブのコードを検索

4. 両ツール使用（Search → Analyst）:
   ソースコードの推論で影響テーブルを特定した後に下流ジョブを展開する場合に使用。
   - 「〇〇ファイルのカラム変更で改修・テストが必要な機能を教えて」
     → まずsource_code_search_toolでソースコードを読み
       カラム処理→変数名→書き込み先テーブルをLLMが推論し、
       次にimpact_analysis_toolで特定したテーブルを使う下流ジョブを展開
   - 「〇〇処理でINSERTされているテーブルを参照している他のジョブは？」
     → Search でソースコードからINSERT先テーブルを特定 →
       Analyst でそのテーブルを使う他ジョブを検索

【ツール順序の判断基準】
- 質問の出発点が「テーブル名・ジョブ名・フラグ」→ Analyst から始める
- 質問の出発点が「ファイル内容・カラム処理・処理ロジック・エラー」→ Search から始める
- 2つ目のツールが必要かどうかは1つ目の結果を見てから判断する
...
| 解説/説明/処理内容/フロー/どんな処理/概要 | Search のみ | 意味的な内容説明が目的 |
| エラー/障害/原因/異常終了/調査/対処 | Search のみ | 推論・診断が目的 |
| ～を使っているプログラムはどんな処理？ | Search のみ | 件数でなく処理内容が目的 |

**チェーン判定（どちらを先に呼ぶかの判断基準）**

| 質問の起点 | → 開始ツール | → 次のツール | 判断根拠 |
|---|---|---|---|
| テーブル名・ジョブ名・フラグ値が明示されている | **Analyst** 先 | Search（必要なら） | 構造検索で対象を絞ってからコード説明 |
| ファイル内容・カラム処理・処理ロジック・エラー文 | **Search** 先 | Analyst（必要なら） | コード推論で対象テーブルを特定してから下流展開 |
