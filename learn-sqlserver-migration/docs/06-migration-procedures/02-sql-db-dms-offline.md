...
2. 「移行シナリオの選択」画面で以下を選択して「選択」：

| 項目 | 設定値 |
|---|---|
| ソースの種類 | SQL Server |
| ターゲットの種類 | Azure SQL Database |
| Database Migration Service | Database Migration Service を選択 |

3. 「Data Migration Service の作成」画面で以下を設定：
...
1. ポータル → 作成した DMS インスタンスの概要画面 → **「New migration」** をクリック
2. 「移行シナリオの選択」で以下を設定して「選択」：
...
3. 「Azure SQL Database オフライン移行ウィザード」を順に進めます：

### ① ソースの詳細

| 項目 | 設定値 |
|---|---|
| ソース SQL Server の詳細 | オンプレSQL Serverのサーバー名またはIPアドレス |
| 認証の種類 | SQL 認証 |
| ユーザー名 | sa（またはdb_datareaderロールのユーザー） |
| パスワード | 対応するパスワード |

「次へ：ソースSQL Serverへの接続」

### ② ソースSQL Serverへの接続

接続情報を確認し「次へ：移行するデータベースの選択」

### ③ 移行するデータベースの選択

対象のDBにチェックを入れて「次へ：ターゲットの Azure SQL Database への接続」

### ④ ターゲットの Azure SQL Database への接続
...
「次へ：ソースとターゲットのデータベースのマッピング」

### ⑤ ソースとターゲットのデータベースのマッピング

移行元DBと移行先DBを対応づけて「次へ」

### ⑥ スキーマとテーブルの選択

「**不足しているスキーマの移行**」チェックボックスを **✅ オン** にします。
これにより、テーブル・ビュー・ストアドプロシージャ・インデックス等のスキーマオブジェクトが
データ移行前に自動的に作成されます。

> ⚠️ **移行されるスキーマオブジェクト（主なもの）**
> スキーマ・テーブル・インデックス・ビュー・ストアドプロシージャ・
> ユーザー定義関数・ユーザー定義型・ロール・ルール など

移行するテーブルを選択（「すべてのテーブルを選択」または個別選択）。

### ⑦ データベース移行の概要 → 移行の開始

内容を確認して「**移行の開始**」をクリック。

> ⚠️ **「移行の開始」をクリックした時点がダウンタイムの起点です**
...
|---|---|
| Creating | 移行を開始中 |
| Preparing for copy | ターゲットのオートスタット・トリガー・インデックスを無効化中 |
| Copying | データをコピー中 |
| Copy finished | コピー完了、インデックス再構築待ち |
| Rebuilding indexes | インデックスを再構築中 |
| Succeeded | 完了 |
...
Azure SQL Database への DMS 移行は内部で Azure Data Factory を使用しており、以下の制限があります。

| 制限 | 内容 |
| --- | --- |
| オンライン移行 | **非対応**（オフラインのみ） |
| テーブル数上限 | 1DB あたり 100,000 テーブルまで |
| 計算列 | 移行されない（手動で対応が必要） |
| 2バイト文字のテーブル名 | 非対応（移行前にリネームが必要） |
| テーブル数が多い場合 | 起動オーバーヘッドにより速度が低下する場合あり |
| 大きなBLOBカラム | タイムアウトで失敗する場合あり |
...
DMS の監視画面でエラー内容を確認し、そのテーブルだけ
SqlPackage で補完する方法もあります。
