...
**概要**: 方式 B と見た目の構成（DMS + SHIR）は同じだが、ターゲットが Azure SQL Database の場合は内部で Azure Data Factory パイプラインが使われ、SHIR がソースからターゲットへ直接データをコピーする（バックアップ経由ではない）。SHIR はオンプレまたは Azure VNet 内 VM に設置可能。
...
> この方式の違いにより、**方式B（SQL MI向け）とは異なり Storage Account は不要**（バックアップファイルを Blob に中継する工程が存在しないため）。
