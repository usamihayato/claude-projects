...
STEP 5   DataMigration リソースプロバイダーの登録         ← Azure ポータル（初回のみ）

【移行実行フェーズ（方式で分岐）】
────── オフライン（ネイティブ RESTORE） ────────────────────────
STEP 6A  フルバックアップ取得（WITH CHECKSUM）
STEP 7A  AzCopy で Blob Storage にアップロード
STEP 8A  SQL MI に接続 → SAS 認証情報の作成 → RESTORE DATABASE FROM URL
STEP 9A  リストア進捗の監視

────── オンライン（LRS：Log Replay Service） ──────────────────
STEP 6B  完全復旧モデルへの変更確認
STEP 7B  フルバックアップ + 差分バックアップの取得
STEP 8B  バックアップを Blob Storage にアップロード
STEP 9B  LRS を PowerShell で開始
STEP 10B ログバックアップを定期取得・アップロード（移行中継続）
STEP 11B カットオーバー実行

【確認・切り替えフェーズ（共通）】
STEP 12  動作確認（件数突合・クエリ確認）
STEP 13  READ_ONLY 設定（参照専用運用のため）
STEP 14  後片付け（不要リソース削除）
```
...
## STEP 5｜DataMigration リソースプロバイダーの登録（初回のみ）

1. ポータル → 「サブスクリプション」→ 対象を選択
2. 「リソースプロバイダー」→ `Microsoft.DataMigration` を検索
3. 「登録」→ 状態が `Registered` になるまで待つ（数分）

---

## オフライン移行（STEP 6A〜9A）：ネイティブ BACKUP / RESTORE FROM URL（推奨）
...
## STEP 8A｜SQL MI で RESTORE DATABASE FROM URL を実行
...
## オンライン移行（STEP 6B〜11B）：LRS（Log Replay Service）PowerShell 版
...
## STEP 10B｜ログバックアップを定期取得・アップロード（カットオーバーまで継続）
...
| SAS トークンが取得された | ☐ |
| Microsoft.DataMigration が登録された | ☐ |
