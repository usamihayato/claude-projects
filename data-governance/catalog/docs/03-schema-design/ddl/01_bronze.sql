...
CREATE TABLE IF NOT EXISTS DG_CATALOG.META.SOURCE_SHEET (
    ...
    SOURCE_SYSTEM_ID          NUMBER          REFERENCES DG_CATALOG.META.SOURCE_SYSTEM(SOURCE_SYSTEM_ID),
    FILE_NAME                 VARCHAR(500)    NOT NULL,      -- 元の Excel ファイル名
    SHEET_NAME                VARCHAR(200)    NOT NULL,      -- シート名（目次シートに記載された名称）
    TABLE_PHYSICAL_NAME       VARCHAR(200),                  -- 対応するテーブル物理名（判明している場合）
    COLUMN_PHYSICAL_NAME      VARCHAR(200),                  -- 対応するカラム物理名（判明している場合）
    SHEET_TYPE                VARCHAR(50),                   -- シートの種別（区分値一覧 / 列定義 / その他）
    IS_ACTIVE                 BOOLEAN         DEFAULT TRUE,  -- 処理対象かどうか
    REMARKS                   VARCHAR(1000),
    CREATED_AT                TIMESTAMP_NTZ   DEFAULT CURRENT_TIMESTAMP(),
