...
| 観点 | 説明 |
|---|---|
| Spark シャッフルデータ | 1 TB スキャン時に `/opt/owl/spark/work/` 配下に大量の一時ファイルが生成される |
| ログの肥大化 | `${OWL_BASE}/logs/` が長期運用で数 GB 〜 数十 GB に達することがある |
...
> ただし `/opt/owl/spark/work/` 配下の Spark 一時ファイルの定期削除を運用で担保すること。
...
grep -E "agent|metastore|polling" "${OWL_BASE}/logs/owl-agent.log" | tail -20
...
> **重要**: アップグレード前に必ず **VM ディスクのスナップショット**を取得すること。  
> Metastore はグループ会社管理のため自社でバックアップ操作は行わない。  
> ロールバックは Disk Snapshot からの VM 復元で対応する（`bin.backup` 方式より確実）。
...
> Metastore のバックアップはグループ会社側で管理。アップグレード前にグループ会社へバックアップ取得を依頼すること。
...
アップグレード後に問題が発生した場合、アップグレード前に取得した **Disk Snapshot** から VM を復元する。
