...
| Metastore | PostgreSQL Flexible Server GP_D4ds_v5（4 vCPU / 16 GB） | 1 | メタストア DB |
...
| Metastore | PostgreSQL Flexible Server GP_D4ds_v5 | 1 | メタストア DB |
...
### 2.2 Azure DB for PostgreSQL Flexible Server（Japan East）

| SKU | vCPU | RAM | 時間単価（JPY） | 月額 コンピュート（JPY） |
|---|---|---|---|---|
| Burstable B2ms | 2 | 8 GB | ¥19.25 | ¥14,053 |
| **GP_D4ds_v5** | **4** | **16 GB** | **¥62.62** | **¥45,713** |
| GP_D8ds_v5 | 8 | 32 GB | ¥125.24 | ¥91,425 |
...
**PostgreSQL GP_D4ds_v5 合計月額の途中式:**
```
コンピュート: ¥62.62/hr × 730 hr = ¥45,713
ストレージ（32 GB）: ¥698
合計: ¥45,713 + ¥698 = ¥46,411/月
```

> 本試算では最小構成として GP_D4ds_v5 + 32 GB ストレージを採用。  
> DQ のスキーマ初期化後のメタストアサイズは通常 1〜5 GB 程度であり 32 GB で十分。
...
| ACR Basic | ¥551 | ¥0.755/日 × 30日（ストレージ別） |
| ACR ストレージ（10 GB） | ¥138 | ¥0.023/GB/月 × 10 GB × 150 JPY |
...
| 2 | Metastore | PostgreSQL GP_D4ds_v5 + 32 GB | ¥62.62 × 730h + ¥698 | ¥46,411 |
| 3 | OS Disk | Premium SSD P10（128 GB）× 1 | 固定 | ¥2,529 |
| 4 | Data Disk | Premium SSD P20（512 GB）× 1 | 固定 | ¥7,546 |
| 5 | ネットワーク送受信 | アウトバウンド 10 GB/月 想定 | ¥16.5/GB × 10 GB | ¥165 |
| **合計** | | | | **¥188,022/月** |
...
```
月額: ¥188,022
年額（×12）: ¥188,022 × 12 = ¥2,256,264
5年総額（×60）: ¥188,022 × 60 = ¥11,281,320
```
...
| 4 | Metastore | PostgreSQL GP_D4ds_v5 + 32 GB | ¥62.62 × 730h + ¥698 | ¥46,411 |
| 5 | OS Disk（System Pool） | Premium SSD P10 × 1 | 固定 | ¥2,529 |
| 6 | OS Disk（DQ Pool） | Premium SSD P10 × 1 | 固定 | ¥2,529 |
| 7 | PVC（DQ data） | Premium SSD P10 × 1 | 固定 | ¥2,529 |
| 8 | ACR | Basic Tier + 10 GB | 基本 + ストレージ | ¥1,097 |
| 9 | Load Balancer | Standard × 1（1ルール） | ¥2.75 × 730h + ¥0.275 × 730h | ¥2,209 |
| 10 | ネットワーク送受信 | アウトバウンド 10 GB/月 想定 | ¥16.5/GB × 10 GB | ¥165 |
| **合計** | | | | **¥199,841/月** |
...
```
月額: ¥199,841
年額（×12）: ¥199,841 × 12 = ¥2,398,092
5年総額（×60）: ¥199,841 × 60 = ¥11,990,460
```
...
| 4 | Metastore | PostgreSQL GP_D4ds_v5 + 32 GB | ¥62.62 × 730h + ¥698 | ¥46,411 |
| 5 | OS Disk（全ノード） | Premium SSD P10 × 5 | ¥2,529 × 5台 | ¥12,645 |
| 6 | Load Balancer | Standard × 1（ARO 付属分） | ¥2.75 × 730h + ¥0.275 × 730h | ¥2,209 |
| 7 | ネットワーク送受信 | アウトバウンド 10 GB/月 想定 | ¥16.5/GB × 10 GB | ¥165 |
| **合計** | | | | **¥507,827/月** |
...
```
月額: ¥507,827
年額（×12）: ¥507,827 × 12 = ¥6,093,924
5年総額（×60）: ¥507,827 × 60 = ¥30,469,620
```
...
| 構成 | 月額（JPY） | 年額（JPY） | 5年総額（JPY） | Standaloネ比 |
|---|---|---|---|---|
| **Standalone（Azure VM）** | **¥188,022** | **¥2,256,264** | **¥11,281,320** | 1.0× |
| AKS | ¥199,841 | ¥2,398,092 | ¥11,990,460 | 1.06× |
| ARO | ¥507,827 | ¥6,093,924 | ¥30,469,620 | 2.70× |

### 6.2 コスト構成の内訳（月額）

```
【Standalone】月額 ¥188,022
  VM (E16s_v5)     ¥131,371  ████████████████████████████  69.9%
  PostgreSQL        ¥46,411  █████████                     24.7%
  ディスク           ¥10,075  ██                             5.4%
  ネットワーク          ¥165  ▏                              0.1%

【AKS】月額 ¥199,841
  VM (E16s_v5 DQ) ¥131,371  ████████████████████████████  65.7%
  PostgreSQL        ¥46,411  █████████                     23.2%
  VM (D2s_v5 Sys)  ¥11,001  ██                             5.5%
  ディスク            ¥7,587  █                              3.8%
  ACR/LB/NW         ¥3,471  ▏                              1.7%

【ARO】月額 ¥507,827
  Worker VM ×2    ¥262,742  ████████████████████████████  51.7%
  Master VM ×3    ¥144,235  ███████████████               28.4%
  ARO クラスター料金 ¥39,420  ████                           7.8%
  PostgreSQL        ¥46,411  █████                          9.1%
  ディスク/NW        ¥15,019  ██                             3.0%
```
...
|---|---|---|---|
| **月額コスト** | ¥188,022 ★ | ¥199,841 | ¥507,827 |
| **5年総額** | ¥11,281,320 ★ | ¥11,990,460 | ¥30,469,620 |
| **AKSとのコスト差（月）** | −¥11,819 | 基準 | +¥307,986 |
...
| 観点 | 推奨構成 | 理由 |
|---|---|---|
| **コスト最小** | **Standalone** | 月額 ¥188,022 で最安。AKS比 −¥11,819/月、ARO比 −¥319,805/月 |
| **5年 TCO** | **Standalone** | ARO比で約 ¥19,188,300（約1,919万円）安い |
...
以下のオプションを適用すると、いずれの構成でもコストを大幅に削減できる。

### 7.1 Reserved Instances（予約インスタンス）

VM と PostgreSQL を 1年・3年の予約購入にすることで割引を受けられる。

| リソース | 割引率（1年） | 割引率（3年） |
|---|---|---|
| VM（E16s_v5） | 約 36% | 約 58% |
| PostgreSQL Flexible Server | 約 36% | 約 58% |

**Standalone での Reserved Instance 適用時の月額試算（1年予約）:**

```
VM (E16s_v5): ¥131,371 × (1 - 0.36) = ¥84,077
PostgreSQL:   ¥46,411 × (1 - 0.36) = ¥29,703
ディスク:      ¥10,075（変化なし）
ネットワーク:     ¥165（変化なし）
合計: ¥84,077 + ¥29,703 + ¥10,240 = ¥124,020/月
```

| 期間 | 従量課金 | 1年予約 | 削減額 |
|---|---|---|---|
| 月額 | ¥188,022 | ¥124,020 | −¥64,002（−34%） |
| 年額 | ¥2,256,264 | ¥1,488,240 | −¥768,024 |
| 5年総額 | ¥11,281,320 | ¥7,441,200 | −¥3,840,120 |

### 7.2 Spot VM / スポットノード（AKS のみ）

スキャンが週1回・夜間実行であれば、Spark Executor をスポット VM で実行することで最大 90% 削減できる。ただし中断リスクあり。
...
|---|---|---|---|
| Standalone | ¥11,281,320 | ¥7,441,200 | −¥3,840,120 |
| AKS | ¥11,990,460 | ¥7,880,400 | −¥4,110,060 |
| ARO | ¥30,469,620 | ¥22,047,120 | −¥8,422,500 |

> ARO クラスター料金（¥39,420/月）は予約割引の対象外のため、VM・PostgreSQL のみに適用。
...
# PostgreSQL Flexible Server GP_D4ds_v5 (Japan East, JPY)
curl -s "https://prices.azure.com/api/retail/prices?\
api-version=2023-01-01-preview&\
\$filter=armRegionName eq 'japaneast' \
and currencyCode eq 'JPY' \
and priceType eq 'Consumption' \
and contains(skuName,'D4ds') \
and serviceName eq 'Azure Database for PostgreSQL'" \
| python3 -c "
import json,sys
data=json.load(sys.stdin)
for i in data['Items']:
